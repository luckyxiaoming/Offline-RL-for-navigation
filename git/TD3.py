...
class DoneNetwork(eqx.Module):
    ...
    def __init__(self, key: PRNGKeyArray, obs_size: int):
        ...
        last_layer = nn.Linear(in_features=256, out_features=1, key=keys[0])
        last_layer = eqx.tree_at(
            where=lambda l: l.weight,
            pytree=last_layer,
            replace_fn=lambda w: w * 1e-5
...
class TD3(object):
    def __init__(self, args, key):
        ...
        self.donenetwork = DoneNetwork(key=keys[3], obs_size=obs_size)
        ...
        self.donenetwork_target = DoneNetwork(key=keys[6], obs_size=obs_size)
        ...
        done_scheduler = optax.cosine_decay_schedule(
                            init_value=args.learning_rate,
                            decay_steps=args.total_offline_steps/2,
                            alpha= 0.2 )
        ...
        self.donenetwork_optimizer = optax.chain(
            optax.clip_by_global_norm(10),
            optax.adam(learning_rate=Q_scheduler))
    ...
    def save_model(self):
        ...
        eqx.tree_serialise_leaves("done_model.eqx", self.donenetwork)
    ...
    def load_model(self):
        ...
        self.donenetwork = eqx.tree_deserialise_leaves("done_model.eqx", self.donenetwork)
    ...
    @staticmethod
    @eqx.filter_jit
    def update_done_network(action_high, action_low, policy_noise, 
                            noise_clip, gamma,
                            actor_target: Actor,
                            qf1_target: QNetwork,
                            donenetwork: DoneNetwork,
                            donenetwork_optimizer, 
                            donenetwork_optimizer_state: optax.OptState,
                            observation, goal_observation, action, reward, done,
                            next_observation, key: jax.random.PRNGKey):
        ...
        # q_function targets
        next_q1 = qf1_target(next_observation, goal_observation, next_actions)
        current_q1 = qf1_target(observation, goal_observation, action)
      
 
        y = reward + (1 - jnp.squeeze(done)) * gamma * next_q1

        #  Loss
        def loss_d_fn(donenetwork: DoneNetwork):
            loss =  current_q1 - (reward + gamma * (1 - jnp.squeeze(donenetwork(next_observation, goal_observation)))*next_q1)
            return jnp.square(loss).mean()

        loss_donenetwork, grad_donenetwork = eqx.filter_value_and_grad(loss_d_fn)(donenetwork)

        update_donenetwork, donenetwork_optimizer_state = donenetwork_optimizer.update(grad_donenetwork, donenetwork_optimizer_state, eqx.filter(donenetwork, eqx.is_array))
        donenetwork = eqx.apply_updates(donenetwork, update_donenetwork)

        return donenetwork, donenetwork_optimizer_state, loss_donenetwork, key
    ...
    def train(self, observation, goal_observation, action, next_observation, reward, done):
        ...
        # Update done network
            self.donenetwork, self.done_optimizer_state, self.loss_donenetwork, self.key = self.update_done_network(
                action_high=self.args.env_params['action_space_high'],
                action_low=self.args.env_params['action_space_low'],
                policy_noise=self.args.policy_noise, noise_clip=self.args.noise_clip,
                gamma=self.args.gamma,
                ...
                donenetwork=self.donenetwork,
                donenetwork_optimizer=self.donenetwork_optimizer,
                donenetwork_optimizer_state=self.done_optimizer_state,
                observation=observation, goal_observation=goal_observation, action=action, reward=reward,
                done=done, next_observation=next_observation,key=self.key
            )
    ...
    for step in range(total_offline_steps):
        ...
        if args.track:
            wandb.log({
                    ...
                    'done_network_loss': td3.loss_donenetwork,  
