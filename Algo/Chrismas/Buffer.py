...
class DynamicHERBuffer:
    def __init__(self, args: Args):
        ...
        if hasattr(args, 'training_dataset_filepath') and args.training_dataset_filepath:
            ...
            for file in args.training_dataset_filepath:
                steps_1 = self._load_file(file, current_idx)
                current_idx += steps_1
        else:
    ...
    def _load_file(self, filepath, start_global_idx):
        ...
        try:
            with h5py.File(filepath, "r") as f:
                ...
                raw_feats = f["features"][:]
                # Simple Normalization
    
                acts = f["actions"][:, 0:3]  # Assuming first 3 are lx, ly, rx
    ...
    def prepare_goal_data(self, target_file_path):
        ...
        try:
            with h5py.File(target_file_path, "r") as f:
                ...
                for i in range(len(raw_feats)):
                    goal = jnp.array(raw_feats[i], dtype=jnp.float32)
                            
                
                
                    send = jnp.array(raw_feats[i], dtype=jnp.float32)
                    self.S_goal.append(send.reshape(-1))
        ...
        print(f"GOAL DATA:  goal have been loaded.")
