...
def main(args):
    ...
    for step in range(total_offline_steps):
        ...
        if (step) % 5000 == 0 and step > 0:
            ...
            fqe.reset_stats()
            fqe_update_steps = 20
            for i in range(fqe_update_steps):  # FQE update steps
                ReplayBuffer.batch_size = 512*10
                obs, g_obs, act, next_obs, rew, done, mask = ReplayBuffer.sample_policy_batch()
                ...
                if not i == fqe_update_steps -1:
                    fqe.add_batch(z=obs_Q_feature, next_z=next_obs_Q_feature, rewards=rew, done=done)
                #fqe_loss = fqe.linear_regression(z=obs_Q_feature, next_z=next_obs_Q_feature, rewards=rew, done=done)
                if i == fqe_update_steps -1:
                    w_norm = fqe.solve()
                    fqe_loss = fqe.calculate_loss(z=obs_Q_feature, next_z=next_obs_Q_feature, rewards=rew, done=done)
            ...
            obs_sg = jnp.array(ReplayBuffer.S_goal)
            N = obs_s0.shape[0]
            M = obs_sg.shape[0]
            s0_new = jnp.repeat(obs_s0, M, axis=0)
            sg_new = jnp.tile(obs_sg, (N,1))
            obs_s0 = s0_new
            obs_sg = sg_new
            ...
            if args.track:
                    wandb.log({
                      ...
                      #  'Metrics/Expert Discounted Return mean': expert_returns_mean,
                        ...
                        'Metrics/FQE Weight Norm': w_norm,
...
if __name__ == "__main__":
  ...
  #  main(args)
    ...
    args.exp_name = 'spare+HER_reward'
    args.alpha = 0.001
    args.env_id = 'realrobot-2Hz-0.001bc' 
    main(args)

    args = tyro.cli(Args)
    args.exp_name = 'spare+HER_reward'
    args.alpha = 0.01
    args.env_id = 'realrobot-2Hz-0.01bc' 
    main(args)
